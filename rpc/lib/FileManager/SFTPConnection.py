...
class SFTPConnection(object):
    ...
    def create(login, server_id, logger=None):
        ...
        logger.info("Database created and opened successfully file = %s" %
                    DB_FILE)
        ...
        try:
            cursor.execute(
                "SELECT * FROM sftp_servers WHERE fm_login = ? AND id = ?",
                (login, server_id))
            ...
            connection = SFTP(hostname=params['host'],
    ...
    def get_error(e, msg="", logger=None):
        if logger is not None:
            logger.error("Error in FTP: %s, %s, traceback = %s" %
