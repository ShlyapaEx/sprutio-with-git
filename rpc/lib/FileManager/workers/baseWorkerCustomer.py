...
class BaseWorkerCustomer(BaseWorker):
    ...
    def preload(self, root=False):
        if not root:
            ...
            if not p.authenticate(self.login, self.password):
                raise Exception('Not Authenticated - %s (%s) ("%s":"%s"):' %
                                (p.code, p.reason, self.login, self.password))
        ...
        self.logger.info(
            "%s process started PID = %s , Title = %s" %
            (str(self.__class__.__name__), str(self.pid), str(process_title)))
    ...
    def get_abs_path(path=''):
        ...
        :return: string
        """
        abs_path = os.path.join(ROOT_MOUNT,
                                BaseWorkerCustomer.normalize_path(path))
        return abs_path
