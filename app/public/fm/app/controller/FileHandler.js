...
Ext.define("FM.controller.FileHandler", {
  ...
  openDirectory: function (session, path, panels) {
    ...
    FM.Logger.log(
      "Event openDirectory run in FileHandler! data = ",
      session,
      path,
      panels
    );
    ...
    for (i = 0, len = panels.length; i < len; i++) {
      ...
      if (panel.filelist.hasListCache(path)) {
        ...
        results.push(
          this.fireEvent(
            FM.Events.file.listFiles,
            panel.filelist.getListCache(path),
            [panel]
          )
        );
      } else {
          ...
          FM.backend.ajaxSend("/actions/files/list", {
            ...
            success: (function (_this) {
              return function (response) {
                ...
                response_data = Ext.util.JSON.decode(
  ...
  listFiles: function (listing, panels) {
    var i, len, panel, results;
    FM.Logger.log(
      "Event listFiles run in FileHandler! data = ",
      listing,
      panels
    );
    results = [];
  ...
  makeDirectory: function (item, session) {
    FM.Logger.log(
      "Event makeDirectory run in FileHandler! data = ",
      item,
      session
    );
  ...
  removeFiles: function (status, session, progress_window) {
    ...
    if (
      status.status != null &&
      (status.status === FM.Status.STATUS_SUCCESS ||
        status.status === FM.Status.STATUS_ABORT)
    ) {
          ...
          if (status.data.success != null) {
            ...
            for (j = 0, len1 = ref1.length; j < len1; j++) {
              ...
              if (FM.helpers.IsSubpathOf(panel.session, file)) {
                FM.Actions.Open.execute(
                  panel,
                  FM.helpers.GetRootPath(panel.session)
                );
                break;
              } else {
                results.push(void 0);
      ...
      if (status.data.errors != null && status.data.errors.length > 0) {
        FM.helpers.ShowError(
          Ext.util.Format.format(
            t("Unable to remove {0} elements."),
      ...
      status.status != null &&
      status.status === FM.Status.STATUS_ERROR
    ) {
      progress_window.hide();
      FM.Logger.info("Operation error", status);
      return FM.helpers.ShowError(
        t("Error during operation. Please contact Support.")
      );
    } else {
      return FM.helpers.ShowError(
        t("Unknown operation status. Please contact Support.")
      );
    }
  },
  ...
  analyzeSize: function (status, session, chart_window) {
    ...
    if (
      ...
      chart_series.eachRecord(function (record) {
        FM.Logger.debug(
          "Chart series record color",
          record,
          this.getLegendColor(index)
        );
        chart_colors.push({
          item: record,
