...
Ext.define("FM.view.windows.CreateArchiveWindow", {
  ...
  listeners: {
    ...
    beforeshow: {
      fn: function () {
        FM.Logger.debug(
          "FM.view.windows.CreateArchiveWindow beforeshow() called",
          this,
          arguments
        );
        Ext.ComponentQuery.query(
          "textfield[name=archive-file-name]",
          this
        )[0].setValue("archive_" + FM.helpers.DateTimestamp());
        return Ext.ComponentQuery.query(
          "textfield[name=archive-path-name]",
          this
        )[0].setValue(this.getSession().path);
      },
    },
  },
  initComponent: function () {
    FM.Logger.debug(
      "FM.view.windows.CreateArchiveWindow initComponent() called",
      arguments
    );
    this.create_btn = Ext.create("Ext.button.Button", {
      ...
      handler: (function (_this) {
        return function (button, e) {
          ...
          FM.Logger.debug(
            "CreateArchive create handler called()",
            arguments,
            _this
          );
          if (_this.create != null) {
            path = Ext.ComponentQuery.query(
              "textfield[name=archive-path-name]",
              _this
            )[0].getValue();
            file = Ext.ComponentQuery.query(
              "textfield[name=archive-file-name]",
              _this
            )[0].getValue();
            filepath = path === "/" ? path + file : path + "/" + file;
            type = Ext.ComponentQuery.query(
              "radio[name=type]",
              _this
            )[0].getGroupValue();
    ...
    this.cancel_btn = Ext.create("Ext.button.Button", {
      ...
      handler: (function (_this) {
        return function (button, e) {
          FM.Logger.debug(
            "CreateArchive Cancel handler called()",
            arguments,
            _this
          );
    ...
    this.close_btn = Ext.create("Ext.button.Button", {
      ...
      handler: (function (_this) {
        return function () {
          FM.Logger.debug(
            "CreateArchive Close handler called()",
            arguments,
            _this
          );
          return _this.close();
