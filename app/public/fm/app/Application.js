...
Ext.define("FM.Application", {
  ...
  isAllowed: function (action, panel, files) {
    ...
    if (
      allowed_no_files[action_name] != null &&
      allowed_no_files[action_name]
    ) {
    ...
    if (files.length === 1) {
      if (
        panel.actions[action_name] != null &&
        panel.actions[action_name] === true
      ) {
        if (FM.Actions.ExtractArchive.self.getName() === action_name) {
          return (
            (ref = files[0].get("ext")) === "zip" ||
            ref === "rar" ||
            ref === "7z" ||
            ref === "gz" ||
            ref === "bz2" ||
        ...
        if (
          ((ref1 = files[0].get("ext")) === "zip" ||
            ...
            ref1 === "tar" ||
            ref1 === "tgz") &&
          (FM.Actions.Edit.self.getName() === action_name ||
            FM.Actions.View.self.getName() === action_name)
        ) {
          return false;
        }
      }
      ...
      if (
        files[0].get("is_dir") &&
        allowed_dir[action_name] == null &&
        !allowed_dir[action_name]
      ) {
        return false;
      }
      if (
        !files[0].get("is_dir") &&
        !files[0].get("is_link") &&
        FM.Actions.Open.self.getName() === action_name
      ) {
        return false;
      }
      if (
        !files[0].get("is_dir") &&
        !files[0].get("is_link") &&
        FM.Actions.AnalyzeSize.self.getName() === action_name
      ) {
        return false;
      }
    }
    if (files.length > 1) {
      if (
        allowed_multiple[action_name] == null ||
        !allowed_multiple[action_name]
      ) {
        return false;
      }
      if (panel.actions != null && panel.actions[action_name] != null) {
  ...
  ParseErrorMessage: function (message, default_message) {
    var cant_create_dir_ftp,
      cant_create_file_ftp,
      file_binary,
    ...
    rename_error_agent_no_file =
      /(.)*(Unable to rename source element No such file or directory)(.)*/;
    ...
    if (message.match(cant_create_file_ftp)) {
      return t(
        "Could not create file - server error.<br/>Check your file or folder permissions"
      );
    }
    if (message.match(cant_create_dir_ftp)) {
      return t(
        "Could not create dir - server error.<br/>Check folder permissions or the dir already exists"
      );
    }
  ...
  CheckOverwrite: function (panel, records, callback) {
    ...
    intersection = Ext.Array.intersect(
      FM.helpers.GetFilesNames(records),
      FM.helpers.GetFilesNames(panel.filelist.store.data.items)
    );
    ...
    question = Ext.create("FM.view.windows.WarningWindow", {
      title: t("Overwrite"),
      msg: Ext.util.Format.format(
        t(
          "{0} items with matching names have been identified.<br/>Would you like to overwrite them?"
        ),
        intersection.length
      ),
  ...
  GetFileExt: function (filename, is_dir) {
    ...
    ext = filename.substring(
      (Math.max(0, filename.lastIndexOf(".")) || filename.length) + 1
    );
    return ext;
  },
  ...
  GetSelected: function (panel) {
    var records;
    records = _.filter(
      panel.filelist.getView().getSelectionModel().getSelection(),
      function (record) {
        if (record.get("name") !== "..") {
          if (record.get("is_link") !== true) {
            return true;
  ...
  SetActivePanel: function (panel, restore_selection, select_default) {
    ...
    FM.Active.selection = FM.Active.filelist
      .getView()
      .getSelectionModel()
      .getSelection();
    ...
    if (!restore_selection) {
      ...
      FM.getApplication().fireEvent(
        FM.Events.main.selectPanel,
        FM.Active,
        FM.helpers.GetSelected(FM.Active)
      );
      return;
    }
    ...
    if (FM.Active.selection != null && FM.Active.selection.length > 0) {
      FM.Active.filelist
        .getView()
        .getSelectionModel()
        .select(FM.Active.selection);
    } else {
      FM.helpers.SelectDefault(FM.Active);
    }
    return FM.getApplication().fireEvent(
      FM.Events.main.selectPanel,
      FM.Active,
      FM.helpers.GetSelected(FM.Active)
    );
  ...
  initConstants: function () {
    ...
    FM.Regex = {};
    FM.Regex.TextFilesExt =
      /^(txt|js|php|cpp|c|py|css|rb|tpl|inc|sh|htaccess|htm|html|xhtml|json|sql|php4|php5)$/i;
